...
class SingleTrack:
    ...
    # ------------------------------------------------------------------
    #  PLATFORM MOTION COMPENSATION (KALMAN SMOOTHING)
    # ------------------------------------------------------------------
    def _compensate_platform_motion(self, frame, cam_x, cam_y, error_x, error_y, env_params):
        """
        Visual phase correlation fails when the background is featureless (e.g. tracking
        a laser beacon in the sky). It ends up tracking the beacon instead of the camera drift!
        
        Instead, we rely on the Kalman Filter's physics model. We tune the Measurement
        Noise (R) dynamically based on the camera jitter amplitude. The Kalman Filter
        filters out the ±20px white noise jitter, and we drive the PTZ motors using 
        the Kalman's smoothed state prediction. This perfectly stabilizes the line of sight.
        """
    ...
    def update(
        ...
        if managed:
            if matched_detection is not None:
                _, _, best_confidence, best_contour = matched_detection
        ...
        if status_str in ["TRACKING", "DISTURBED"] and best_contour is not None:
            ...
            if M["m00"] != 0:
                ...
                # Drive PTZ motors using the SMOOTHED Kalman state, not the jittered raw measurement!
                smooth_world_x = self.kf.statePost[0, 0]
                smooth_world_y = self.kf.statePost[1, 0]

                raw_error_x = smooth_world_x - (cam_x + self.cam_width // 2)
                raw_error_y = smooth_world_y - (cam_y + self.cam_height // 2)
                error_x, error_y = self._compensate_platform_motion(
                    frame, cam_x, cam_y, raw_error_x, raw_error_y, env_params
                )
        ...
        if status_str == "LOST":
            ...
            path_map = {
                "random": "random", "circular": "circular", "spiral": "spiral",
                "figure8": "figureof8", "figureof8": "figureof8",
                "sinusoidal": "sinusoidal", "straightline": "straightline",
            }
            path_key = path_map.get(path_key, "")

            # === PHASE 1: 0 - 0.3s (Frames 1-9) -> Kalman Coasting ===
            if self.lost_frames < 10:
                # Just use Kalman filter's pre-calculated prediction
                # It has Constant Acceleration, so let it naturally curve!
                pred_world_x = self.kf.statePre[0, 0]
                pred_world_y = self.kf.statePre[1, 0]
                
                # Sync post state with pre state to keep Kalman moving forward
                for i in range(6):
                    self.kf.statePost[i, 0] = self.kf.statePre[i, 0]
            ...
            # === PHASE 2: 0.3 - 1.0s (Frames 10-30) -> GRU Prediction ===
            elif self.lost_frames < 30:
                if self.lost_frames == 10:
                    # Initialize GRU at the start of Phase 2
                    ...
                    if path_key in self.gru_models and len(self.measurement_history) >= 2:
                        ...
                        self.gru_full_buffer = list(self.measurement_history[-120:]) + self.gru_sequence.copy()
                        
                        if obstacles and len(obstacles) > 0:
                            self.reacq_point = self._find_reacquisition_point(self.gru_sequence, obstacles)
                ...
                # Use GRU if available, otherwise fallback to Kalman
                if self.gru_sequence and len(self.gru_sequence) > 0:
                    ...
                    for i in range(4): self.kf.statePost[i, 0] = self.kf.statePre[i, 0]

                    if self.reacq_active and self.reacq_point is not None:
                        # Add micro-search (small spiral/wobble) to the reacquisition point
                        wobble_x = int(math.sin(self.lost_frames * 0.5) * 10)
                        wobble_y = int(math.cos(self.lost_frames * 0.5) * 10)
                        raw_error_x = int(self.reacq_point[0] - (cam_x + self.cam_width // 2)) + wobble_x
                        raw_error_y = int(self.reacq_point[1] - (cam_y + self.cam_height // 2)) + wobble_y
                    else:
                        lookahead = self._get_lookahead(env_params, path_key)
                        intercept_pt = self._find_interception_point(self.gru_sequence, cam_x, cam_y, lookahead)
                        if intercept_pt:
                            raw_error_x = int(intercept_pt[0] - (cam_x + self.cam_width // 2))
                            raw_error_y = int(intercept_pt[1] - (cam_y + self.cam_height // 2))
                        else:
                            raw_error_x = int(pred_world_x - (cam_x + self.cam_width // 2))
                            raw_error_y = int(pred_world_y - (cam_y + self.cam_height // 2))
                else:
                    ...
                    pred_world_x = self.kf.statePre[0, 0]
                    pred_world_y = self.kf.statePre[1, 0]
                    for i in range(6): self.kf.statePost[i, 0] = self.kf.statePre[i, 0]
                    raw_error_x = int(pred_world_x - (cam_x + self.cam_width // 2))
                    raw_error_y = int(pred_world_y - (cam_y + self.cam_height // 2))
            ...
            # === PHASE 3: > 1.0s (Frames 30+) -> Active Search ===
            else:
                # Target is deeply lost. Initiate expanding spiral search from last known coasting point.
                # Stop Kalman from advancing further to prevent it from wandering off forever.
                for i in range(6): self.kf.statePost[i, 0] = self.kf.statePre[i, 0]
                
                search_frame = self.lost_frames - 30
                radius = min(150, 10 + search_frame * 2)  # expanding radius up to 150px
                angle = search_frame * 0.3  # rotate outwards
                
                search_x = self.kf.statePre[0, 0] + radius * math.cos(angle)
                search_y = self.kf.statePre[1, 0] + radius * math.sin(angle)
                
                raw_error_x = int(search_x - (cam_x + self.cam_width // 2))
                raw_error_y = int(search_y - (cam_y + self.cam_height // 2))
...
class KalmanTracker:
    ...
    def update(self, frame, cam_x, cam_y, current_path="random", obstacles=None, env_params=None):
        if env_params is None:
            env_params = {}
        if obstacles is None:
            obstacles = []
        ...
        for i, track in enumerate(self.tracks):
            ...
            t_ex, t_ey, t_rmse, t_status, t_log = track.update(frame, cam_x, cam_y, current_path, obstacles=obstacles, env_params=env_params, managed=True, matched_detection=matched)
        ...
        if len(self.tracks) == 1:
            ...
            if len(active_tracks) == 2:
                ...
                if dist > 500:
                    ...
                    avg_rmse = (getattr(t1, 'last_rmse', 0.0) + getattr(t2, 'last_rmse', 0.0)) / 2.0
                    return int(e_x), int(e_y), avg_rmse, "TRACKING (DUAL)", None
    ...
    @property
    def kf(self):
        return self.tracks[0].kf

    @property
    def gru_sequence(self):
        return getattr(self.tracks[0], "gru_sequence", None)

    @property
    def reacq_point(self):
        return getattr(self.tracks[0], "reacq_point", None)
